const fs = require("fs");
/* Exporto Contenedor hacia productos.js */
...
class Contenedor {
    ...
    try {
      ...
      if (!this.exists(this.archivo)) {
        ...
        if (this.readFile(this.archivo)) {
          ...
          if (data.length === 0) {
            console.log("Archivo vacio, se agrega producto con id: 1");
      ...
      console.log(`Hubo un error al agregar el producto: ${error.message}`);
    }
  }
  ...
  async change(id, info) {
    try {
      /* verifico si existe el archivo */
      if (this.exists(this.archivo)) {
        let data = await this.readFile(this.archivo);
        /* busco id de producto a modificar */
